...
void COSA_AQ6370B::DoubleToByte(double dblValue, BYTE *pbyValue, BYTE byLenght)
{
	BYTE byCount, byIndex = 0;

	for (size_t i = byLenght; i > 0; i--)
	{
		byCount = (i - 1) * 8;
		pbyValue[byIndex] = (BYTE)((DWORD)dblValue >> byCount);
		byIndex++;
	}
}
void COSA_AQ6370B::ByteTODouble(BYTE *pbyValue, double *pdblValue, BYTE byLenght)
{
	BYTE byCount, byIndex = 0;
	double dblValue = 0;

	for (size_t i = 0; i < byLenght; i++)
	{
		byCount = (byLenght - i - 1) * 8;
		dblValue += pbyValue[i] << byCount;
	}
	*pdblValue = dblValue / 1000.0;
}
...
//Time:2019/7/26
//////////////////////////////////////////////////////
DWORD COSA_AQ6370B::InitalAmpForMode2(double dblStartWL, double dblStopWL, double dblRBW)
{
	DWORD dwErrorCode = SUCCESS;
	CHAR szCmdString[TXLENS];
	CHAR szDataStream[RXMINLENS];
...
DWORD COSA_AQ6370B::InitAmpTest(double dblStartWL, double dblStopWL, double dblRBW)
{
	...
	try
	{
		...
		{
			dwErrorCode = InitalAmpForMode2(dblStartWL, dblStopWL, dblRBW);
			if (dwErrorCode != SUCCESS)
			{
				throw ERROR_OSAINITAMPMODE2;
			}
		}
	}
	catch (DWORD dwError)
...
void COSA_AQ6370B::GetOSAFile(pOTMSFileData pstData, LPCTSTR strFolderName)
{
	if (!m_DefFileObj.ReadOTMSFileDataFromBinFile(strFolderName, pstData))
	{
	...
	{
		m_stScanSetting.m_bExcludeSourceASE += pstData->stFileHeader.bChopMode[i];
		m_stScanSetting.m_bIncludeShotNosie += pstData->stFileHeader.bShotNoise[i];
		m_stScanSetting.m_bInterpolationMode += pstData->stFileHeader.bASEALGO[i];
		//m_stScanSetting.m_bNFCalculationRBW += pstData->stFileHeader.bChopMode[i];
	}

	ByteTODouble(pstData->stFileHeader.bFittingArea, &m_stScanSetting.m_dblInterpolationVal, 2);
	ByteTODouble(pstData->stFileHeader.bStartWL, &m_stScanSetting.m_dblStartWL, 4);
	ByteTODouble(pstData->stFileHeader.bStopWL, &m_stScanSetting.m_dblStopWL, 4);
	ByteTODouble(pstData->stFileHeader.bRBW, &m_stScanSetting.m_dblRBW, 2);
	ByteTODouble(pstData->stFileHeader.bResOffset, &m_stScanSetting.m_dblSrcOffset, 2);
	ByteTODouble(pstData->stFileHeader.bRawOffset, &m_stScanSetting.m_dblAmpOffset, 2);

}

void COSA_AQ6370B::SetOSAFile(stOTMSFileData stData, LPCTSTR strFolderName)
{
	DoubleToByte(SWEEPPOINT, stData.stFileHeader.bSMPL, 2);
	DoubleToByte(m_stScanSetting.m_bExcludeSourceASE, stData.stFileHeader.bChopMode, 2);
	DoubleToByte(m_stScanSetting.m_bIncludeShotNosie, stData.stFileHeader.bShotNoise, 2);
	DoubleToByte(m_stScanSetting.m_bInterpolationMode, stData.stFileHeader.bASEALGO, 2);
	//	DoubleToByte(m_stScanSetting.m_bNFCalculationRBW, stData.stFileHeader.bAvgTime, 2);
	DoubleToByte(m_stScanSetting.m_dblInterpolationVal * 1000, stData.stFileHeader.bFittingArea, 2);
	DoubleToByte(m_stScanSetting.m_dblStartWL * 1000, stData.stFileHeader.bStartWL, 4);
	DoubleToByte(m_stScanSetting.m_dblStopWL * 1000, stData.stFileHeader.bStopWL, 4);
	DoubleToByte(m_stScanSetting.m_dblRBW * 1000, stData.stFileHeader.bRBW, 2);
	DoubleToByte(m_stScanSetting.m_dblSrcOffset * 1000, stData.stFileHeader.bResOffset, 2);
	DoubleToByte(m_stScanSetting.m_dblAmpOffset * 1000, stData.stFileHeader.bRawOffset, 2);

	if (!m_DefFileObj.WriteOTMSFileData2Binary(strFolderName, stData))
	{
...
DWORD COSA_AQ6370B::MeasureSourceForMode1()
{
	...
	try
	{
		...
		dwErrorCode = GetOSARBW(m_dblSRCRBW, 'A');
		if (dwErrorCode != SUCCESS)
		{
			throw ERROR_OSAMEARSOUREMODE1;
		}
		...
		sprintf(szFile, "%s","./SRCF.bin");
		SetOSAFile(stData, szFile);
...
//Time:2019/7/25
////////////////////////////////////////////////////////////////////////
DWORD COSA_AQ6370B::MeasureSourceForMode2(LPCSTR lpstrFilePath)
{
	DWORD dwErrorCode = SUCCESS;
	CHAR szCmdString[TXLENS];
	...
	try
	{
		...
		ZeroMemory(szSaveFileName, MAX_PATH);
		sprintf(szSaveFileName, "%s\\SRCF.bin", lpstrFilePath);
		if (!m_DefFileObj.WriteOTMSFileData2Binary(szSaveFileName, stData))
		{
			throw ERROR_OSAMEARSOUREMODE2;
		}
...
DWORD COSA_AQ6370B::MeasureSource()
{
	...
	try
	{
		...
		else if (m_nNFLib == 2)//模式2
		{
			dwErrorCode = MeasureSourceForMode2(m_szAmpFilePath);
			if (dwErrorCode != SUCCESS)
			{
				throw ERROR_OSAMEARSOUREMODE2;
			}
...
DWORD COSA_AQ6370B::MeasureAmplifierForMode1(BOOL bScanSrcOnly)
{
	...
	try
	{
		...
		stOTMSFileData stData;
		stCALFFileData  m_stCALFFileData;
		stOACFFileData  m_stOACFFileData;

		ZeroMemory(&stData, sizeof(stOTMSFileData));
		ZeroMemory(&m_stCALFFileData, sizeof(stCALFFileData));
		ZeroMemory(&m_stOACFFileData, sizeof(stOACFFileData));
		...
		sprintf(szFile, "%s", FILE_NAME_AMP);
		SetOSAFile(stData, szFile);

		sprintf(szFile, "%s", FILE_NAME_SRC);
		GetOSAFile(&stData, szFile);
		...
		memset(&m_stCALFFileData, 0, sizeof(m_stCALFFileData));
		DoubleToByte((double)m_stEDFAResult.m_wChCount, m_stCALFFileData.bWLCount, 4);
		m_stCALFFileData.pdblOutOffset = new double[m_stEDFAResult.m_wChCount];
		m_stCALFFileData.pdblWL = new double[m_stEDFAResult.m_wChCount];
		m_stCALFFileData.pdblInOffset = new double[m_stEDFAResult.m_wChCount];

		m_stOACFFileData.pdbASELeftOffset = new double[m_stEDFAResult.m_wChCount];
		m_stOACFFileData.pdblASERightOffset = new double[m_stEDFAResult.m_wChCount];
		m_stOACFFileData.pdblWL = new double[m_stEDFAResult.m_wChCount];

		for (i = 0; i < m_stEDFAResult.m_wChCount; i++)
		{
			m_stCALFFileData.pdblOutOffset[i] = m_stEDFAResult.m_pdblAmpPwr[i];
			m_stCALFFileData.pdblWL[i] = m_stEDFAResult.m_pdblChWL[i];
			m_stCALFFileData.pdblInOffset[i] = m_stEDFAResult.m_pdblSrcPwr[i];
			m_stOACFFileData.pdbASELeftOffset[i] = m_stEDFAResult.m_pdblAmpPwr[i];
			m_stOACFFileData.pdblASERightOffset[i] = m_stEDFAResult.m_pdblSrcPwr[i];
			m_stOACFFileData.pdblWL[i] = m_stEDFAResult.m_pdblChWL[i];
		}

		sprintf(szFile, "%s", FILE_NAME_CALF);
		m_DefFileObj.WriteCALFFileData2Binary(szFile, m_stCALFFileData);

		sprintf(szFile, "%s", FILE_NAME_CALF);
		m_DefFileObj.ReadCALFFileDataFromBinFile(szFile, &m_stCALFFileData);

		DoubleToByte((double)m_stEDFAResult.m_wChCount, m_stOACFFileData.bWLCount, 4);
		
		sprintf(szFile, "%s", FILE_NAME_OACF);
		m_DefFileObj.WriteOACFFileData2Binary(szFile, m_stOACFFileData);

		sprintf(szFile, "%s", FILE_NAME_OACF);
		m_DefFileObj.ReadOACFFileDataFromBinFile(szFile, &m_stOACFFileData);

		delete[]m_stCALFFileData.pdblWL;
		delete[]m_stCALFFileData.pdblOutOffset;
		delete[]m_stCALFFileData.pdblInOffset;
		delete[]m_stOACFFileData.pdbASELeftOffset;
		delete[]m_stOACFFileData.pdblASERightOffset;
		delete[]m_stOACFFileData.pdblWL;
		m_stCALFFileData.pdblWL = NULL;
		m_stCALFFileData.pdblOutOffset = NULL;
		m_stCALFFileData.pdblInOffset = NULL;
		m_stOACFFileData.pdbASELeftOffset = NULL;
		m_stOACFFileData.pdblASERightOffset = NULL;
		m_stOACFFileData.pdblWL = NULL;
...
//Time:2019/7/26
///////////////////////////////////////////////////////////////////////////
DWORD COSA_AQ6370B::MeasureAmplifierForMode2(BOOL bScanSrcOnly)
{
	DWORD dwErrorCode = SUCCESS;
	char seps[] = ",\t\n";
	char* token;
	...
	try
	{
		...
		sprintf(szFileName, "%s\\AMPF.bin", m_szAmpFilePath);

		if (!m_DefFileObj.WriteOTMSFileData2Binary(szFileName, stData))
		{
			throw ERROR_OSAMEARAMPMODE1;
		}
		...
		if (!GetAdoEDFAResult(m_stScanSetting, &m_stEDFAResult, m_pdblScanData[SRC_SCAN], m_pdblScanData[AMP_SCAN], stData.stRawData.pdblRBW, SWEEPPOINT))
		{
			throw ERROR_SWEEP_DATA;
		}
...
DWORD COSA_AQ6370B::MeasureAmplifier(BOOL bScanSrcOnly)
{
	...
	try
	{
		...
		{
			dwErrorCode = MeasureAmplifierForMode2(bScanSrcOnly);
			if (dwErrorCode != SUCCESS)
			{
				throw ERROR_OSAMEARAMPMODE2;
			}
		}
	}
	catch (DWORD dwError)
	...
	double *pdblGainFlatness, double *pdblMaxNF, double* pdblMaxGain, double* pdblMinGain)
{
	...
	try
	{
		...
		{
			dwErrorCode = CalculateMode2(pwChCount, pdblChWL, pdblSrcpwr,
...
DWORD COSA_AQ6370B::CalculateMode2(DWORD *pwChCount, double *pdblChWL, double *pdblSrcpwr,
	double *pdblChGain, double *pdblChNF, double *pdblChAse,
	double *pdblGainFlatness, double *pdblMaxNF, double* pdblMaxGain, double* pdblMinGain)
{
	DWORD dwErrorCode = SUCCESS;
	...
	try
	{
		...
		if (!m_DefFileObj.ReadOTMSBinFileHeader(strFile, &stSrcData.stFileHeader))
		{
			throw ERROR_OSASRCFILE;	
		}
		WORD wSampleCount = stSrcData.stFileHeader.bSMPL[0] << 8;
		wSampleCount += stSrcData.stFileHeader.bSMPL[1];

		stSrcData.stRawData.pdblWL = (double*)VirtualAlloc(NULL, sizeof(double)*wSampleCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		stSrcData.stRawData.pdblPower = (double*)VirtualAlloc(NULL, sizeof(double)*wSampleCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		stSrcData.stRawData.pdblRBW = (double*)VirtualAlloc(NULL, sizeof(double)*wSampleCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		stSrcData.stRawData.nPointCount = wSampleCount;

		if (!m_DefFileObj.ReadOTMSFileDataFromBinFile(strFile, &stSrcData))
		{
			throw ERROR_OSAAMPCFILE;
		}

		sprintf(strFile, "%s", FILE_NAME_AMP);
		if (!m_DefFileObj.ReadOTMSBinFileHeader(strFile, &stAmpData.stFileHeader))
		{
			throw ERROR_OSASRCFILE;
		}
		wSampleCount = stAmpData.stFileHeader.bSMPL[0] << 8;
		wSampleCount += stAmpData.stFileHeader.bSMPL[1];
		stAmpData.stRawData.pdblWL = (double*)VirtualAlloc(NULL, sizeof(double)*wSampleCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		stAmpData.stRawData.pdblPower = (double*)VirtualAlloc(NULL, sizeof(double)*wSampleCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		stAmpData.stRawData.pdblRBW = (double*)VirtualAlloc(NULL, sizeof(double)*wSampleCount, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		stAmpData.stRawData.nPointCount = wSampleCount;

		if (!m_DefFileObj.ReadOTMSFileDataFromBinFile(strFile, &stAmpData))
		{
			throw ERROR_OSASRCFILE;
		}

		if (stSrcData.stRawData.nPointCount != stAmpData.stRawData.nPointCount)
		{
			throw ERROR_SWEEP_DATA;
		}

		for (i = 0; i < m_dwSweepPoint; i++)
		{
			m_pdblScanData[SRC_SCAN][i] = stSrcData.stRawData.pdblPower[i];
			m_pdblScanData[AMP_SCAN][i] = stAmpData.stRawData.pdblPower[i];
		}

		if (!GetAdoEDFAResult(m_stScanSetting, &m_stEDFAResult, m_pdblScanData[SRC_SCAN], m_pdblScanData[AMP_SCAN], stAmpData.stRawData.pdblRBW, SWEEPPOINT))
		{
			throw ERROR_SWEEP_DATA;
		}
		//读取信号通道增益
	...
	m_DefFileObj.RealseAllocOSAScanRawData(stSrcData.stRawData);
	m_DefFileObj.RealseAllocOSAScanRawData(stAmpData.stRawData);
...
{
	DWORD  dwErrorCode = SUCCESS;
	TCHAR	tszDataStream[90000]; //nScanCount* 16 byte
	...
	try
	{
		nTempLength = 16 * nScanCount;
		if (pszDataStream == NULL)
		{
			pszDataStream = (char*)VirtualAlloc(NULL, nTempLength, MEM_RESERVE, PAGE_READWRITE);
...
//Time：2019/7/26
//////////////////////////////////////////////////////////
DWORD COSA_AQ6370B::SetMeasureAmpFilePath(char * szAmpFilePath)
{
	DWORD  dwErrorCode = SUCCESS;
	if (strlen(szAmpFilePath) > MAX_PATH)
	{
		dwErrorCode = EEROR_OSASETAMPFILEPATH;
		return dwErrorCode;
	}
	ZeroMemory(m_szAmpFilePath, MAX_PATH);
	strcpy(m_szAmpFilePath, szAmpFilePath);
	return dwErrorCode;
}
