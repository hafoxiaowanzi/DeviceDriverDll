...
public:
	...
	virtual DWORD SetOSASenseMode(int nMode) = 0;
};
...
public:
	...
	DWORD SetMeasureAmpFilePath(char * szAmpFilePath);
	DWORD GetEdfaChannelRbw(double *pdblChRbw);
	DWORD SetAmpModeAseAlgo(int iFitSel, int iFitAlgoSel, double dblAseFitArea, int iFitPointDisplaySel);
	DWORD SetAmpModeNfCalc(int iRbwSel, int iShotNoiseSel);

	//Set the start wavelength and stop wavelength
...
//	BOOL RunSweep(double* pdblSweepDataX,double* pdblSweepDataY,char szSelTrace);//perform a sweep and get the level axis data of specified trace.
	...
	DWORD SetTraceAttribute(char szSelTrace, USHORT uiAttribute);//set the attribute of the given 
	DWORD GetOSARBW(double *pdblRBW, int nScanCount);
...
protected:
	...
	DWORD InitalAmpForMode2(double dblStartWL, double dblStopWL, double dblRBW);
	DWORD MeasureSourceForMode0();
	DWORD MeasureSourceForMode1();
	DWORD MeasureSourceForMode2(LPCSTR lpstrFilePath);
	DWORD MeasureAmplifierForMode0(BOOL bScanSrcOnly = FALSE);
	DWORD MeasureAmplifierForMode1(BOOL bScanSrcOnly = FALSE);
	DWORD MeasureAmplifierForMode2(BOOL bScanSrcOnly = FALSE);
	...
	void SetOSAFile(stOTMSFileData pstData, LPCSTR strFolderName);
	void GetOSAFile(pOTMSFileData pstData, LPCSTR strFolderName);
	DWORD CalculateMode2(DWORD *pwChCount, double *pdblChWL, double *pdblSrcpwr,
