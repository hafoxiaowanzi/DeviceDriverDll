...
void CTestUDLDlg::OnBtnOpen() 
{
	...
	strConfigFile.Format("%s\\UDL.Config.xml",szBuffer);
...
void CTestUDLDlg::OnBtnTlsParameters() 
{
	...
	UpdateData(TRUE);
	m_pTLS->SetTLSParameters(m_nTLSChIndex,FALSE, m_fTlsPower, m_fTlsWav);
	AfxMessageBox(m_pTLS->GetLastMessage());
}
...
void CTestUDLDlg::OnBtnPmWl() 
{
	...
	UpdateData(TRUE);
	m_pPM->SetWavelength(m_nPMChIndex, m_fPmWav);
...
void CTestUDLDlg::OnBtnPmGetwl() 
{
	...
	UpdateData(TRUE);
	m_pPM->GetWavelength(m_nPMChIndex, &m_fPmWav);
	CString strValue;
