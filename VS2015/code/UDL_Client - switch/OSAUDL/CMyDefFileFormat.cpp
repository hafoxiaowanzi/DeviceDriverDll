...
void CCMyDefFileFormat::DoubleToByte(double dblValue, BYTE *pbyValue, BYTE byLenght)
{
...
void CCMyDefFileFormat::ByteTODouble(BYTE *pbyValue, double *pdblValue, BYTE byLenght)
{
	BYTE byCount, byIndex = 0;
	double dblValue = 0;
...
BOOL CCMyDefFileFormat::WriteOTMSRawData2CSV(LPCSTR lpstrCSVFile, stOTMSFileData stFileData)
{
	...
	fprintf(fpFile, "\n");
	//WL
	for (int i = 0;i < wSampleCount;i+=4)
	{
...
DWORD CCMyDefFileFormat::CalculateOTMSRawDataCRC32(stOSAScanRawData stData)
{
	...
	stbData.bWLArry= (BYTE*)VirtualAlloc(NULL,sizeof(char)*stData.nPointCount *4,MEM_RESERVE,PAGE_READWRITE);
	stbData.bPowerArry = (BYTE*)VirtualAlloc(NULL, sizeof(char)*stData.nPointCount * 4, MEM_RESERVE, PAGE_READWRITE);
	stbData.bRBWArry = (BYTE*)VirtualAlloc(NULL, sizeof(char)*stData.nPointCount * 4, MEM_RESERVE, PAGE_READWRITE);
	int j = 0;
	DWORD dwTempValue;
	for (int i = 0;i < stData.nPointCount * 4;i += 4)
	{
...
BOOL CCMyDefFileFormat::WriteOTMSFileData2Binary(LPCSTR lpstrFileName, stOTMSFileData stFileData)
{
	...
	try
	{
		...
		hFile = (HANDLE)CreateFile(lpstrFileName, GENERIC_WRITE| GENERIC_READ, FILE_SHARE_WRITE| FILE_SHARE_READ, NULL, CREATE_ALWAYS, 0, NULL);
		...
		//stFileData.stFileHeader.bSMPL[0] = stFileData.stRawData.nPointCount >> 8;
		//stFileData.stFileHeader.bSMPL[1] = stFileData.stRawData.nPointCount;
		...
		bWritePtr = (BYTE*)VirtualAlloc(NULL, nRawDataBYTELength * 3, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		...
		dwStLength = nRawDataBYTELength * 3;
		writesize = 0;
		if (!WriteFile(hFile, bWritePtr, dwStLength, &writesize, NULL))
		{
			ZeroMemory(szMsg, MAX_PATH);
...
BOOL CCMyDefFileFormat::ReadOTMSFileDataFromBinFile(LPCSTR lpstrFileName, pOTMSFileData pData)
{
	...
	try
	{
		...
		bReadPtr = (BYTE*)VirtualAlloc(NULL, wSampleCount * 4 * 3, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
		dwReadLength = wSampleCount * 4 * 3;
		...
		for (int i = 0; i < wSampleCount * 4; i += 4)
		{
			...
			int iFlag1Index = wSampleCount * 4;
			...
			pData->stRawData.pdblPower[j] = (double)(*((long*)&dwTempValue)) / 1000.0;

			int iFlag2Index = wSampleCount * 4 * 2;
...
BOOL CCMyDefFileFormat::GetOSAFile(pOTMSFileData pstData, LPCSTR strFolderName, stScanSetting	&stScanSetting)
{
	if (!ReadOTMSFileDataFromBinFile(strFolderName, pstData))
	{
		return FALSE;
	}

	//memset(&stScanSetting, 0, sizeof(stScanSetting));
	stScanSetting.m_bExcludeSourceASE = 0;
	stScanSetting.m_bIncludeShotNosie = 0;
	stScanSetting.m_bInterpolationMode = 0;
	for (size_t i = 0; i < 2; i++)
	{
		stScanSetting.m_bExcludeSourceASE += pstData->stFileHeader.bChopMode[i];
		stScanSetting.m_bIncludeShotNosie += pstData->stFileHeader.bShotNoise[i];
		stScanSetting.m_bInterpolationMode += pstData->stFileHeader.bASEALGO[i];
		//m_stScanSetting.m_bNFCalculationRBW += pstData->stFileHeader.bChopMode[i];
	}

	ByteTODouble(pstData->stFileHeader.bFittingArea, &stScanSetting.m_dblInterpolationVal, 2);
	ByteTODouble(pstData->stFileHeader.bStartWL, &stScanSetting.m_dblStartWL, 4);
	ByteTODouble(pstData->stFileHeader.bStopWL, &stScanSetting.m_dblStopWL, 4);
	ByteTODouble(pstData->stFileHeader.bRBW, &stScanSetting.m_dblRBW, 2);
	ByteTODouble(pstData->stFileHeader.bOffsetIN, &stScanSetting.m_dblSrcOffset, 2);
	ByteTODouble(pstData->stFileHeader.bOffsetOUT, &stScanSetting.m_dblAmpOffset, 2);
		
	return TRUE;
}

BOOL CCMyDefFileFormat::SetOSAFile(stOTMSFileData stData, LPCSTR strFolderName, stScanSetting stScanSetting, int iSweepPoints)
{
	DoubleToByte(iSweepPoints, stData.stFileHeader.bSMPL, 2);
	DoubleToByte(stScanSetting.m_bExcludeSourceASE, stData.stFileHeader.bChopMode, 2);
	DoubleToByte(stScanSetting.m_bIncludeShotNosie, stData.stFileHeader.bShotNoise, 2);
	DoubleToByte(stScanSetting.m_bInterpolationMode, stData.stFileHeader.bASEALGO, 2);
	//	DoubleToByte(m_stScanSetting.m_bNFCalculationRBW, stData.stFileHeader.bAvgTime, 2);
	DoubleToByte(stScanSetting.m_dblInterpolationVal * 1000, stData.stFileHeader.bFittingArea, 2);
	DoubleToByte(stScanSetting.m_dblStartWL * 1000, stData.stFileHeader.bStartWL, 4);
	DoubleToByte(stScanSetting.m_dblStopWL * 1000, stData.stFileHeader.bStopWL, 4);
	DoubleToByte(stScanSetting.m_dblRBW * 1000, stData.stFileHeader.bRBW, 2);
	DoubleToByte(stScanSetting.m_dblSrcOffset * 1000, stData.stFileHeader.bOffsetIN, 2);
	DoubleToByte(stScanSetting.m_dblAmpOffset * 1000, stData.stFileHeader.bOffsetOUT, 2);

	if (!WriteOTMSFileData2Binary(strFolderName, stData))
	{
		return FALSE;
	}
	return TRUE;
...
BOOL CCMyDefFileFormat::FindFolder(LPCSTR strFolderPath)
{
	WIN32_FIND_DATAA findFile;
	HANDLE handle = INVALID_HANDLE_VALUE;
	BOOL bFind = FALSE;

	handle = FindFirstFile(strFolderPath, &findFile);
