...
DWORD COSA_AQ6317B::SetAmpOutSigOffset(double dblOutSigOffset)
{
	...
	{
		ZeroMemory(szCmdString,TXLENS);
		sprintf(szCmdString, "OFOUT%7.2f\r\n", dblOutSigOffset);
		dwErrorCode = m_Comm.WriteDevice(szCmdString,strlen(szCmdString));
		if(SUCCESS != dwErrorCode)
		{
			throw dwErrorCode|ERROR_PW_OFFSET;
...
DWORD COSA_AQ6317B::GetEDFAResult(double *pdblSumSrcSigPwr, double *pdblSumAmpSigPwr, double *pdblSunGain)
{
	...
	dwErrorCode = m_Comm.ReadDevice(szCmdString,TXLENS);
	//ASSERT(szCmdString);
    m_dblInputTotal = atof(szCmdString);
    dwErrorCode = SetTraceAct('B');
	if(dwErrorCode!=SUCCESS)
	{
		throw UNEXPECT_OCCUR;
	...
	dwErrorCode = m_Comm.ReadDevice(szCmdString,TXLENS);
	//ASSERT(szCmdString);
	m_dblOutputSignal = atof(szCmdString);
...
DWORD COSA_AQ6317B::RunCalResult()
{
	...
	try
	{
		...
		token = strtok(NULL,szSpec);
		//ASSERT(token); //波长
		m_dblSRCPointWL[nCount-1] = atof(token);
		m_dblAmpPointWL[nCount-1] = atof(token);
		token = strtok(NULL,szSpec);
		//ASSERT(token); //Inupt
		token = strtok(NULL,szSpec);
		m_dblSRCPointPower[nCount-1] = atof(token);
		//ASSERT(token); //OutPut
		token = strtok(NULL,szSpec);
		m_dblAmpPointPower[nCount-1] = atof(token);
		m_dblOutputSignal = dbTomw(m_dblAmpPointPower[nCount-1] );
		//ASSERT(token); //ASE
		token = strtok(NULL,szSpec);
		m_dblASE[nCount-1] = atof(token);
		//ASSERT(token); //RES
		token = strtok(NULL,szSpec);
		m_dblResoln[nCount-1] = atof(token);
		//ASSERT(token); //Gain
		token = strtok(NULL,szSpec);
		m_dblCHGain[nCount] = atof(token);
		//ASSERT(token); //NF
		m_dblCHNF[nCount-1] = atof(token);
