...
DWORD CTLSDll::SetTLSLock(long lChIndex, int nTimeOut, double dbSetWL)
{
	DWORD dwErrorCode = SUCCESS;
	if (m_pTLS != NULL)
	{
		dwErrorCode = ((CTLSControl *)m_pTLS)->SetTLSLock(lChIndex, nTimeOut, dbSetWL);
	}
	else
	{
		dwErrorCode = UNEXPECT_OCCUR;
...
DWORD CTLSDll::SetTLSUnlock(long lChIndex)
{
	DWORD dwErrorCode = SUCCESS;
	if (m_pTLS != NULL)
	{
		dwErrorCode = ((CTLSControl *)m_pTLS)->SetTLSUnlock(lChIndex);
	}
	else
	{
		dwErrorCode = UNEXPECT_OCCUR;
