...
BEGIN_MESSAGE_MAP(CTestUDLDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BTN_OSA_INIT_WDM, OnBtnOsaInitWdm)
	ON_BN_CLICKED(IDC_BTN_OSA_SCANWDM, OnBtnOsaScanwdm)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CTestUDLDlg::OnBtnOsaInitWdm() 
{
	// TODO: Add your control notification handler code here
	if (m_pEngine == NULL)
	{
	...
	UpdateData();
	m_pOSA->InitOSAWDMTest(m_nOSAChIndex, m_fOSAWav, m_fOSAWavEnd, m_fOsaRBW);
	AfxMessageBox(m_pOSA->GetLastMessage());
}

void CTestUDLDlg::OnBtnOsaScanwdm() 
{
	// TODO: Add your control notification handler code here
		if (m_pEngine == NULL)
	{
	...
	UpdateData();
	m_pOSA->MeasureOSAWDM(m_nOSAChIndex);
	AfxMessageBox(m_pOSA->GetLastMessage());
}
