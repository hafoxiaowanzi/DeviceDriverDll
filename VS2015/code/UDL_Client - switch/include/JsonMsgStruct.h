...
typedef struct STSOCKINITINFO
{
	...
	STSOCKINITINFO operator=(STSOCKINITINFO& info)
	{
		...
		if (info.pszSrvIP)
			if (sizeof(this->pszSrvIP) >= sizeof(info.pszSrvIP))
				memcpy_s(this->pszSrvIP, sizeof(this->pszSrvIP), info.pszSrvIP, sizeof(info.pszSrvIP));
...
struct CommMsgPackage
{
	...
	{
		memcpy_s(strPlatForm, sizeof(char)*LENGTH_256B, srcPack.strPlatForm, sizeof(char)*LENGTH_256B);
		memcpy_s(strSlot, sizeof(char)*LENGTH_256B , srcPack.strSlot, sizeof(char)*LENGTH_256B);
		memcpy_s(strUserID, sizeof(char)*LENGTH_256B , srcPack.strUserID, sizeof(char)*LENGTH_256B);
		memcpy_s(strClientID, sizeof(char)*LENGTH_256B, srcPack.strClientID, sizeof(char)*LENGTH_256B);
		memcpy_s(strIPAddress, sizeof(char)*LENGTH_256B, srcPack.strIPAddress, sizeof(char)*LENGTH_256B);
		memcpy_s(strPort, sizeof(char)*LENGTH_256B, srcPack.strPort, sizeof(char)*LENGTH_256B);
		memcpy_s(strMsgType, sizeof(char)*LENGTH_256B, srcPack.strMsgType, sizeof(char)*LENGTH_256B);
		memcpy_s(strTime, sizeof(char)*LENGTH_256B, srcPack.strTime, sizeof(char)*LENGTH_256B);
			...
			if (srcPack.MsgList != NULL)
			{
				for (int index = 0; index < nMsgCount; index++)
				{
					...
					if (srcPack.MsgList[index].strKey)
					{
						...
						memcpy_s(MsgList[index].strKey, len * sizeof(char), srcPack.MsgList[index].strKey, len * sizeof(char));
					}
					if (srcPack.MsgList[index].strValue)
					{
						len = strlen(srcPack.MsgList[index].strValue) + 1;
						MsgList[index].strValue = new char[len];
						memcpy_s(MsgList[index].strValue, len * sizeof(char), srcPack.MsgList[index].strValue, len * sizeof(char));
					}
				}
			}
		}
